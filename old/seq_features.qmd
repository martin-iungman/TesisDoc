...
hek_df=hek_df %>% group_by(name) %>% summarise(hek_tpm=sum(score.y, na.rm=T), hek_interq_width=sum(interquantile_width))
...
```{r}
shape=read_tsv("Analysis/Tables/full_shape.tsv")
shape_summ=shape  %>% filter(tot_prom>20) %>% group_by(name) %>% summarise(shape=median(norm_entropy), n_shape=n())
prom_df=left_join(prom_df, shape_summ, by=c("seq_id"="name"))
```

```{r}
ggplot(prom_df, aes( sample_specificity_gini, shape, col=sample_specificity_class))+geom_point()
```

### ElemeNT

```{r}
element=readxl::read_excel("External_data/CORE_EPDnew_Aug2023.xlsx", sheet = "human")
names(element)=paste0(names(element), "_element")
element=element %>% mutate(across(-name_element, function(x){ifelse(x=="no", F, T)}))
prom_df=left_join(prom_df, element, by=c("name"="name_element"))

data %>% pivot_longer(ends_with("element"), names_to = "motif", values_to = "presence") %>% group_by(rep,motif, var_rank_sw ) %>% summarise(count=sum(presence)) %>% ggplot(aes(var_rank_sw, count, col=motif, text=motif))+geom_smooth()+facet_wrap(~rep)->p
